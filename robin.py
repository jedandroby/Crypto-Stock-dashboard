...
# Forecasting
    ...
    st.title("Support Vector Machine Asset Predictor")
    ...
    def predict():
        ...
        actual_plot = go.Scatter(x=np.arange(len(y_test)), y=y_test, mode='lines', name='Actual')
        predicted_plot = go.Scatter(x=np.arange(len(y_pred)), y=y_pred, mode='lines', name='Predicted')
        st.plotly_chart(go.Figure(data=[actual_plot, predicted_plot], layout=go.Layout))
...
page_names_to_funcs = {
    ...
    'Machine learning algorithms':ML,'Prophet':pro,'Support Vector Machine': svm
}
